class BaseEntity:
    ...
    def get_fields_with_value_as_str(self):
        ...
        """
        # Empiezo por el id
        cadena = f'{self.idfieldname} = {getattr(self, self.idfieldname)}'

        # Completo con el resto
        for attr, value in self.__dict__.items():
            # Descartar el campo que almacena el nombre del campo "id"
            if str(attr) != "idfieldname" and str(attr) != self.idfieldname:
                ...
                if isinstance(value, BaseEntity):
                    ...
                    # de "id"
                    cadena = f"{cadena} , {str(attr)}id = {getattr(value, value.idfieldname)}"
                else:
